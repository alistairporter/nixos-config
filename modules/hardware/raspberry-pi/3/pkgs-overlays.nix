# adapted from https://github.com/NixOS/nixos-hardware/blob/master/raspberry-pi/4/pkgs-overlays.nix
{ config, lib, ... }:
let
  cfg = config.hardware.raspberry-pi."3".apply-overlays-dtmerge;
...
in {
  options.hardware = {
    raspberry-pi."3".apply-overlays-dtmerge = {
